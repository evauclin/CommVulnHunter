...
class LSTMPhishingDetector:
    ...
    def extract_numerical_features(self, df):
        ...
        for text in df["text"]:
            ...
            suspicious_count = sum(
                1 for word in suspicious_words if word in text_str.lower()
            )
    ...
    def prepare_sequences(self, df, is_training=True):
        ...
        if is_training:
            ...
            print(
                f"  Vocabulaire utilisé: {min(len(self.tokenizer.word_index), self.config['max_vocab_size'])} mots"
            )
    ...
    def build_lstm_model(self):
        ...
        embedding = Embedding(
            input_dim=min(
                len(self.tokenizer.word_index) + 1, self.config["max_vocab_size"] + 1
            ),
    ...
    def train_model(
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                "best_lstm_model.h5", save_best_only=True, monitor="val_loss"
            ),
        ]
    ...
    def evaluate_model(self, X_text_test, X_num_test, y_test):
        ...
        report = classification_report(
            y_test_encoded, y_pred, target_names=labels, digits=4
        )
        print(report)
...
def main():
    ...
    df = detector.load_data(
        "full_merged_dataset_benign_phishing.csv", sample_size=15000
    )
    ...
    for i, (text, pred, prob) in enumerate(
        ...
        print(
            f"Confiance: {'PHISHING' if prob > 0.7 else 'SUSPECT' if prob > 0.3 else 'LÉGITIME'}"
        )
