...
def predict_batch(
    ...
    for i, text in enumerate(texts):
        ...
        sequence[0] = [
            token for token in sequence[0] if token <= len(tokenizer.word_index)
        ]
...
def main(csv_path="../pages/emails_live.csv", model_path=DEFAULT_MODEL_PATH):
    ...
    print(
        f"Résumé: {(original_types_backup != df['type']).sum()}/{len(df)} labels modifiés"
    )
