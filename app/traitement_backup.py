...
def convert_numpy_types(obj):
    ...
    if obj is None:
        ...
        if isinstance(obj, (int, float, bool, str)):
            ...
            return str(
                obj
            )  # Conversion en string seulement pour les types non supportés
...
class IndividualFeedbackRetrainingManager:
    ...
    def _check_model_version(self, api_url, expected_version):
        ...
        try:
            ...
            result = subprocess.run(
                curl_cmd, capture_output=True, text=True, timeout=12
            )
            ...
            if result.returncode == 0:
                try:
                    ...
                    if str(current_version) == str(expected_version):
                        ...
                        print(
                            f"   🔍 Version différente: {current_version} vs {expected_version}"
                        )
                        return False

                except json.JSONDecodeError:
                    print("   ⚠️ Réponse API non-JSON")
                    return False
            else:
    ...
    # Fonction de diagnostic post-déploiement
    def diagnose_deployment_status(self):
        ...
        try:
            ...
            print(
                f"   Métadonnées: {'✅' if metadata_file.exists() else '❌'} {metadata_file}"
            )
            ...
            if metadata_file.exists():
                try:
                    ...
                    print(
                        f"   Dernier déploiement: {metadata.get('deployment_timestamp', 'unknown')}"
                    )
                    print(
                        f"   Dernier feedback: {metadata.get('last_feedback_processed', 'unknown')}"
                    )
                except Exception as e:
            ...
            print("\n💡 RECOMMANDATIONS:")
            print(
                "   1. Vérifiez que l'API FastAPI fonctionne: curl http://localhost:8000/health"
            )
            print(
                "   2. Rechargement manuel: curl -X POST http://localhost:8000/reload-model"
            )
            print("   3. Redémarrage Docker: docker-compose restart fastapi")
            print("   4. Vérifiez les logs Docker: docker-compose logs fastapi")
    ...
    def deploy_retrained_model(
        ...
        try:
            ...
            final_validation = self.validate_feedback_correction(
                test_model, feedback_data, self.current_attempt
            )
            if not final_validation.get("validation_passed", False):
                print("❌ Validation finale échouée avec le modèle sauvegardé")
            ...
            reload_success = self.trigger_api_reload_improved(
                new_version, max_retries=5
            )

            if reload_success:
                print("🎉 DÉPLOIEMENT COMPLET RÉUSSI!")
                print("   ✅ Modèle déployé et rechargé automatiquement")
                print(f"   📊 Version: {current_version} → {new_version}")
            else:
                print("⚠️ DÉPLOIEMENT PARTIEL")
                print(f"   ✅ Modèle déployé (version {new_version})")
                print("   ❌ Rechargement automatique échoué")
                print("   💡 Rechargement manuel requis:")
                print("      curl -X POST http://localhost:8000/reload-model")
        ...
        for attempt in range(max_retries):
            ...
            for api_url in api_urls:
                try:
                    ...
                    health_result = self._test_api_endpoint(
                        health_url, method="GET", timeout=5
                    )
                    ...
                    reload_result = self._test_api_endpoint(
                        reload_url, method="POST", timeout=15
                    )
                    if not reload_result:
                        print("   ❌ Rechargement échoué")
                        continue

                    print("   ✅ Rechargement déclenché")
                    ...
                    if version_check:
                        print("   🎉 RECHARGEMENT VALIDÉ!")
                        print(f"   ✅ Nouvelle version {expected_version} confirmée")
                        return True
                    else:
                        print("   ⚠️ Version non mise à jour")
            ...
            if attempt < max_retries - 1:
                print("   ⏳ Pause de 5 secondes avant nouvelle tentative...")
                time.sleep(5)
